...
layui.define(['form','layer','jquery','application','table','treeGrid'],function(exports){
	...
	var obj ={
		...
		//取下拉菜单并进行回填
		selectBaseAndSetVal : function (url,data,selectid,selectValue,valueFiled,textFiled,callback){
			if(!valueFiled)
			{
				valueFiled="value";
			}
			...
			$.ajax({
				...
				success:function(res){
					if(res.code==application.REQUEST_SUCCESS){
						...
						$("#"+selectid).empty();
						for(var i =0;i<data.length;i++){
							$("#"+selectid).append('<option  value="'+data[i][valueFiled]+'">'+data[i][textFiled]+'</option>');//往下拉菜单里添加元素
						}
